...
class Fruit:
    ...
    def __init__(
        ...
        # If the fruit is a watermelon, scale the image up slightly
        if self.s_x // 16 == 26:
            scale = 1.5
        else:
            scale = 1
        self.sprite = canvas.sprite_sheet.crop(
            (self.s_x, self.s_y, self.s_x + 16, self.s_y + 16))
        self.sprite = self.sprite.resize((int(self.canvas.fruit_size * scale),
                                          int(self.canvas.fruit_size * scale)),
                                         Image.Resampling.NEAREST)
        if flip_image:
            self.sprite = self.sprite.transpose(
    ...
    def delete(self, _):
        ...
        if not self.canvas.paused:
            ...
            self.canvas.score += self.canvas.streak * 0.2 * \
                max(10, (0.1 * (self.canvas.m_vel[0] **
                    2 + self.canvas.m_vel[1]**2)**0.5))
            # Generates two halves of chopped fruit
            # The slice sprites can always be found on the 6th row hence
            # s_y is 16 * (6-1)
            left = ChoppedFruit((self.s_x, 5 * 16),
                                (self.canvas.coords(self.object)[
                                 0] - 16, self.canvas.coords(self.object)[1]),
                                (0.2 * self.canvas.m_vel[0] - 50,
    ...
    def pack(self):
        ...
        the save file
        """
        return [(self.s_x, self.s_y), (self.x, self.y),
                (self.velocity_x, self.velocity_y), None, self.flip_image]
