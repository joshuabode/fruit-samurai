...
class Game(Canvas):
            ...
            # Only spawn bombs if the player scores above 5,000
                ...
                bomb = Bomb(
                    (randint(self.fruit_size, self.width - self.fruit_size),
                     self.height - self.fruit_size), (uniform(-3, 3) * self.ppm,
                                                      uniform(-6.64, -4) * self.ppm), self)
    ...
    def pause(self, key):
        ...
        if self.paused:
            self.pause_label = Label(
                self, text="Paused", font=self.heading_font, bg=self.background_color, fg='black')
            self.pause_text = self.create_window(
                self.width / 2, self.height / 2 - 30, anchor='center', window=self.pause_label)
            self.save_button = Button(
                self, text="Save Game", command=self.save_game, bg=self.background_color, highlightbackground=self.background_color)
            self.save_window = self.create_window(
                self.width / 2, self.height / 2, anchor='center', window=self.save_button)
            self.resume_button = Button(self, text="Resume", command=lambda: self.pause(
                None), bg=self.background_color, highlightbackground=self.background_color)
            self.resume_window = self.create_window(
                self.width / 2, self.height / 2 + 30, anchor='center', window=self.resume_button)
            self.lead_button = Button(
                self, text="Leaderboard", command=leaderboard, highlightbackground='#f0d7a1')
            self.lead_window = self.create_window(
                self.width / 2, self.height / 2 + 60, anchor='center', window=self.lead_button)
    ...
    def save_game(self):
        vars = (self.lives, self.score, self.streak, list(self.hit_or_miss), [
                f.pack() for f in self.fruits], [b.pack() for b in self.bombs])
    ...
    def game_over(self):
        game_over_label = Label(
            self, text="Game Over", font=self.heading_font, bg=self.background_color, fg='black')
        ...
        lead_button = Button(self, text="Leaderboard",
                             command=leaderboard, highlightbackground='#f0d7a1')
        ...
        save_score = Button(self, text="Save Score to Leaderboard",
                            command=self.save_score, highlightbackground='#f0d7a1')
        self.create_window(self.width / 2, self.height / 2 + 30,
                           anchor='center', window=save_score)
        restart = Button(
            self, text="Exit", command=self.window.destroy, highlightbackground='#f0d7a1')
