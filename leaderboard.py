...
def leaderboard():
    ...
    scorelist = [score for score in scorelist if score[2] != 'True']    # Removes cheaters from the leaderboard
    scorelist.sort(key=lambda x: x[1], reverse=True)                    # Sort in descending order of score
    title = Label(leaderboard, text="Leaderboard", font=heading_font)
    title.grid(row=0, column=0, columnspan=2)
    for i in range(10):
        try:
            name_l = Label(leaderboard, text=f"{i+1}. {scorelist[i][0]}", font=retro_font)
            name_l.grid(row=i+1, column=0, sticky='w', padx=20)
            score_l = Label(leaderboard, text=str(scorelist[i][1]), font=retro_font)
